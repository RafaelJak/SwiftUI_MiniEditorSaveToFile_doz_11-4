...
struct TextViewSlider: View {
    
    @State var textSlider:Double = 0.0
    var body: some View {
       Text("\(textSlider)")
    }
...
struct DrawView: View {
    ...
    @State var colorObj          = Color.green
     //==========
    var body: some View {
            
                VStack{
                  TextViewSlider(textSlider: sliderValue)
                ...
                .overlay(SettingsMenu(objColor: $objColor, sliderValue:$sliderValue ).opacity(settingsIsVisible),alignment: .bottomTrailing)
